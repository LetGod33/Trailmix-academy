...
import {
  ...
  Calculator, FlaskConical, Landmark, Globe2, ChevronLeft, RotateCcw, Loader2
} from "lucide-react";
...
export default function TrailmixAcademy() {
  ...
  // Load saved progress on mount (from this browser's local storage)
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(STORAGE_KEY);
      if (raw) {
        const data = JSON.parse(raw);
        const mergedProgress = { ...defaultProgress(), ...(data.progress || {}) };
        setProgress(mergedProgress);
        setHearts(typeof data.hearts === "number" ? data.hearts : MAX_HEARTS);
        setXp(data.xp || 0);
        setGems(typeof data.gems === "number" ? data.gems : 12);
  ...
  // Save progress whenever core state changes (after initial load)
  useEffect(() => {
    if (!loaded) return;
    try {
  ...
  }, [progress, hearts, xp, gems, streak, lastVisit, loaded]);
